...
public class ChartCountCommitsLinesAdded extends ChartAnalysis {
    private Map<String, Integer> data;
    private HashMap<String, Integer> modifiedData;

    public ChartCountCommitsLinesAdded(Configuration config) {
    	super("Auteurs", "Added lines");
        ...
        for(var elt : tmp.entrySet())
        	data.put(elt.getKey(), elt.getValue()[0]);
        this.modifiedData = copy(data);
        keepOnlyFirstName(); // On garde que le premier nom de chaque auteur.
    }
    
    public ChartCountCommitsLinesAdded(Configuration config, String startDate, String endDate) {
    	super("Auteurs", "Added lines");
    ...
    public ChartCountCommitsLinesAdded(Configuration config, String date) {
    ...
    public CategoryDataset createDataset() {
        ...
        for(Entry<String, Integer> line : modifiedData.entrySet()) {
            dataset.addValue(line.getValue(),"Nombre de lignes ajoutées",line.getKey());
        }
        return dataset;
    }
